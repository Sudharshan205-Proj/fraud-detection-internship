...
├── app/                    # Streamlit app + model inference service
├── models/                 # Final Random Forest + feature schema
├── r/  tableau/            # R analysis/report and Tableau dashboard
...
| Train & save final Random Forest + feature schema | `python -m src.machine_learning.train_final_model` |
| Visualization datasets (chunked, full dataset) | `python scripts/generate_visualization_data.py` |
| Python charts (Phase 11) | `python scripts/create_visualizations.py` |
| R analysis | `Rscript r/fraud_analysis.R` |
| R visualizations | `Rscript r/fraud_visualization.R` |
| Streamlit application | `streamlit run app/streamlit_app.py` |
...
| [docs/phase-1/](docs/phase-1/data-processing.md) | Per-phase reports: objective, deliverables, commands, status (phases 1-16) |
...
| Phase 11 — Visualization & Tableau | Complete |
| Phase 12 — Testing & Validation | Complete |
| Phase 13 — Deployment | Complete |
| Phase 14 — Documentation | Complete |
| Phase 15 — Final Report & Presentation | In Progress |
| Phase 16 — GitHub Finalization | Pending |

The `app/` folder contains the Streamlit application and model-inference
service, `r/` holds the R analysis, visualizations and R Markdown report,
and `tableau/` holds the Tableau workbook and dashboard documentation.
